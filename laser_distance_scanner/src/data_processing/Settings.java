...
public class Settings {
	 ...
	 * angle_total 		= Der Winkel des Ausschnits auf dem Gemessen wird, von 0 bis x, die ausrichtung im raum wird nicht ber�cksichtigt
	 * angle_angle 		= Der Winkel zwischen 2 direkt nebeneinander liegenden Messungen
	 * angle_tan_array 	= der Tangenz alle Winkel zwischen einer Messunge und einem Beliebigen anzahl pl�tze entfernten winkel. Der Index entspricht die der entfernung zur ausgangsmessung
	 * longVersion		= Es gibt 2 grundlegende Implementierungen, eine die ausschlie�lich mit den Long werten des Sensors arbeitet, die zweite arbeitet mit den Long werten + den daraus berechneten kartesischen koordinaten als punkte
	 */
	...
	/**
	 *   graymap 						= schwarz wei� schatierte karte. dunkel = objekt, hell = frei
	 *  		  						  \-> wird zur erkennung von bewegenden objekten verwendet
	 *  graymap_state 					= aktiv oder inaktiv
	 *  graymap_direct_adding			= aktiv   -> funktion addNewDataDirect: Erstellt eine Momentane karte, die mit den neuen daten bef�llt wird, und die in einem weiteren schritt in die globale karte geupdatet wird
	 *  								  inaktiv -> funktion addNewData:       �nderungen werden direkt in die Globale Karte gespeichert. Dadurch sollen resourcen gespart werden
	 *
	 *   angle 							= winkel -> kreisf�rmig angeordnete vektoren,
	 *  graymap_angle_size				= anzahl an messwerten die in die selbe Graymap spallte projeziert werden
	 *  graymap_angle_steps 			= anzahl an unabh�ngigen spalten in der graymap, berechnung = angle_number / graymap_angle_size
	 *	
	 ...
	 *  graymap_section_size			= gr��e der graymap section in mm 
	 *  graymap_section_steps			= anzahl an unabh�ngigen sectionen in der Graymap, berechnung aus graymap_section_count / graymap_section_size
	 ...
	 *  graymap_move_area_gap_max_Size 	= maximale l�cke zwischen zwei arealen befor diese vereint werden
	 ...
	 *  graymap_gray_Step 				= ein einzelner verdunkelungs schritt von wei� zu grau in den sectionen nach au�en, wegen sich �ffnenden winkeln und daraus entstehenden messfehlern kleiner objekte
	 *
	 ...
	 *  graymap_update_sympel_direction_center  = Wert mit dem bei mergeValueSimpel der wert in richtung 127 abg�ndert wird
	 *  graymap_update_sympel_direction_extreme = Wert mit dem bei mergeValueSimpel der wert in richtung 0 oder Max Gray abg�ndert wird
	 *  graymap_update_sympel                   = Stellt zwischen mergeValueSimpel und mergeValue um, wobei mergeValue die gr��ere Rechenzeit besitzt, jedoch sch�ner funktioniert
	 *
	 ...
	 * 									  \-> sorgt daf�r das station�re objekte lansamer verschwinden und sich bewegende langsamer festsetzen
	...
	private static boolean graymap_direct_adding = true;		// true, direktes einf�gen ist performanter
	...
	private static int graymap_section_size = 500;				// 20 - 500 je gr��er desto besser
	...
	private static double graymap_update_factor =(double)0.05;  // 0.01 - 0.08, nicht zu klein w�hlen, wegen datentyp rundungs problemen
	...
	private static boolean graymap_update_sympel = true;		// true, verwendet eine einfachere berechnung der zusammenf�hrung der karten
	private static boolean graymap_edge_accuracy = true;		// true, deutlich erh�hter rechenaufwand
	private static boolean graymap_visual_server = false;		// false, sehr sehr rechenaufw�ndig!
